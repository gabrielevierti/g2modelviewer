import {
  ...
  v,
  w
} from "./chunk-CJM4VOZO.js";

// node_modules/@gabrielevierti/glyph/lib/glyph.js
function ft(e) {
...
var ut = class {
  ...
  constructor(t) {
    this.face = t, this.coverage = it(ft(t.data), t.width, t.height);
  }
  get lineHeight() {
  ...
  tintedAtlas(t, i) {
    ...
    const o = a.getContext("2d", { alpha: true });
...
var mt = class {
  ...
  constructor(t) {
    for (const i of t.faces)
      this.faces.set(Q2(i.family, i.weight, i.size, i.italic ?? false), new ut(i));
  }
  /** The face for a style, or null if this set was not built for that style. */
  find(t) {
    const i = Number(t.weight ?? 500), n = Math.round(t.size ?? 16);
    return this.faces.get(Q2(dt(t), i, n, t.italic ?? false)) ?? null;
  }
  has(t) {
...
function Rt(e, t, i) {
  return {
    width: e.measure(v(i, t), t.tracking ?? 0),
...
var At = (e) => e.x;
var w2 = (e) => e.x + e.width;
var St = (e) => e.y;
var H2 = (e) => e.y + e.height;
var I2 = (e) => e.x + e.width / 2;
var T2 = (e) => e.y + e.height / 2;
var It = (e) => ({ x: I2(e), y: T2(e) });
function B(e, t) {
...
function zt(e, t) {
  return t.x >= e.x && t.x <= w2(e) && t.y >= e.y && t.y <= H2(e);
}
function Ft(e, t) {
  return !(w2(e) <= t.x || w2(t) <= e.x || H2(e) <= t.y || H2(t) <= e.y);
}
function $t(e, t) {
  ...
  return {
    x: i,
    y: n,
    width: Math.max(0, Math.min(w2(e), w2(t)) - i),
    height: Math.max(0, Math.min(H2(e), H2(t)) - n)
  };
}
function Bt(e, t) {
  const i = Math.min(e.x, t.x), n = Math.min(e.y, t.y);
  return { x: i, y: n, width: Math.max(w2(e), w2(t)) - i, height: Math.max(H2(e), H2(t)) - n };
}
function Mt(e, t, i = "center", n = "middle") {
  const r = i === "left" ? e.x : i === "right" ? w2(e) - t.width : e.x + (e.width - t.width) / 2, s = n === "top" ? e.y : n === "bottom" ? H2(e) - t.height : e.y + (e.height - t.height) / 2;
...
var Yt = (e) => e * 180 / Math.PI;
var _t = (e) => e * Math.PI / 180;
function Z2(e, t, i) {
...
function z(e, t, i, n, r) {
  return i === t ? n : Z2(n + (e - t) / (i - t) * (r - n), Math.min(n, r), Math.max(n, r));
}
var yt = class {
  commands = [];
  moveTo(t, i) {
...
var S2 = "Inter, Helvetica Neue, Helvetica, Arial, sans-serif";
var j2 = "SF Mono, ui-monospace, Menlo, Consolas, monospace";
var m = {
  hero: { font: S2, size: 64, weight: 800, leading: 62 },
  display: { font: S2, size: 46, weight: 800, leading: 46 },
  displaySm: { font: S2, size: 34, weight: 800, leading: 34 },
  headline: { font: S2, size: 22, weight: 700, leading: 26 },
  title: { font: S2, size: 17, weight: 700, leading: 21 },
  body: { font: S2, size: 14, weight: 500, leading: 18 },
  bodyStrong: { font: S2, size: 14, weight: 700, leading: 18 },
  caption: { font: S2, size: 11, weight: 500, leading: 14 },
  label: { font: S2, size: 10, weight: 700, tracking: 1.1, uppercase: true, leading: 13 },
  micro: { font: S2, size: 9, weight: 700, tracking: 0.8, uppercase: true, leading: 11 },
  numeral: { font: j2, size: 15, weight: 600, leading: 18 },
  numeralLg: { font: j2, size: 24, weight: 700, leading: 28 },
  numeralXl: { font: j2, size: 40, weight: 700, leading: 42 }
};
var O2 = { none: 0, xxs: 2, xs: 4, sm: 8, md: 12, lg: 16, xl: 24, xxl: 32, xxxl: 48 };
var A = { none: 0, xs: 3, sm: 5, md: 8, lg: 12, xl: 18, pill: 999 };
var $2 = { width: 576, height: 288 };
var J2 = { x: 14, y: 12, width: 548, height: 264 };
function v2(e) {
  return e.surface === "outline";
}
var gt = 0.18;
var Lt = 4;
var Qt = { gray: h, type: m, space: O2, radius: A, screen: $2, safe: J2, inkBudget: gt, minContrast: Lt };
function xt(e, t, i) {
  const n = e.intrinsic;
...
function tt2(e, t, i, n) {
  ...
  let l = 0, f = 0;
  const u = t.map((M2, p) => {
    const g = M2.size ?? { grow: 1 };
    if (typeof g == "number") {
      const k = D2(g, M2);
      return l += k, k;
    }
    if (g === "auto") {
      const k = D2(xt(M2, o, p), M2);
      return l += k, k;
    }
    return f += g.grow, null;
  }), L = Math.max(0, a - l - c), x = u.map((M2, p) => {
    if (M2 !== null) return M2;
    const g = t[p].size;
    return D2(f > 0 ? L * g.grow / f : 0, t[p]);
  }), b2 = x.reduce((M2, p) => M2 + p, 0) + c, d = Math.max(0, a - b2), C = i.justify ?? "start";
  let V = (n ? r.x : r.y) + (C === "center" ? d / 2 : C === "end" ? d : 0);
  const y = C === "between" && t.length > 1 ? d / (t.length - 1) : C === "around" && t.length > 0 ? d / t.length : 0;
  return C === "around" && (V += y / 2), t.map((M2, p) => {
    const g = x[p], k = M2.cross ?? o, P = M2.align ?? i.align ?? "stretch", G = P === "stretch" && M2.cross === void 0 ? o : k, F2 = P === "center" ? (o - G) / 2 : P === "end" ? o - G : 0, W2 = n ? { x: V, y: r.y + F2, width: g, height: G } : { x: r.x + F2, y: V, width: G, height: g };
    return V += g + s + y, W2;
  });
}
function Ct(e, t, i = {}) {
  return tt2(e, t, i, true);
}
function Ht(e, t, i = {}) {
  return tt2(e, t, i, false);
}
function Ut(e, t, i = {}) {
...
var Jt = (e = 1) => ({ grow: e });
var te = (e, t = {}) => ({ size: "auto", intrinsic: e, ...t });
function ee(e, t = 0) {
  return B(e, t);
}
var et2 = {
...
var it2 = class {
  ...
  duration;
  easing;
  constructor(t, i = 300, n = et2.outCubic) {
...
function ae() {
  return Object.keys(N2).sort();
}
function R2(e, t, i, n, r = 24, s = 15, a = {}) {
  const o = nt2(t);
  if (!o) return;
...
function rt2(e, t, i, n) {
  return e.length === 0 ? [] : e.length === 1 ? [{ x: t.x + t.width / 2, y: z(e[0], i, n, H2(t), t.y) }] : e.map((r, s) => ({
    x: t.x + s / (e.length - 1) * t.width,
    y: z(r, i, n, H2(t), t.y)
  }));
}
function _2(e, t, i, n, r = {}) {
  ...
  for (let l = 0; l < c; l++) {
    const f = c === 1 ? 0.5 : l / (c - 1), u = Math.round(H2(o) - f * o.height);
    e.hline(o.x, w2(o), u, r.gridGray ?? h.hairline), s > 0 && r.format && e.text(r.format(i + (n - i) * f), t.x + s - 5, u, m.micro, r.labelGray ?? h.tertiary, "right", "middle");
  }
  return r.baseline !== false && e.hline(o.x, w2(o), H2(o), h.border), r.labels?.length && r.labels.forEach((l, f) => {
...
function he(e, t, i, n = {}) {
  const [r, s] = Vt(i, n), a = _2(e, t, r, s, n);
  for (const o of i) {
    const c = rt2(o.values, a, r, s);
    if (c.length < 2) continue;
    o.fill !== void 0 && e.scoped((f) => {
      f.clipRect(a);
      const u = [{ x: c[0].x, y: H2(a) }, ...c, { x: c[c.length - 1].x, y: H2(a) }];
      v2(e) ? f.scoped((L) => {
        L.ctx.beginPath(), u.forEach((x, b2) => b2 === 0 ? L.ctx.moveTo(x.x, x.y) : L.ctx.lineTo(x.x, x.y)), L.ctx.closePath(), L.ctx.clip(), L.hatch(a, 6, h.hairline, Math.PI / 4, 1);
...
function ce(e, t, i, n = {}) {
  ...
  const r = n.min ?? Math.min(...i), s = n.max ?? Math.max(...i), a = rt2(i, t, r, s === r ? r + 1 : s);
  n.fill !== void 0 && !v2(e) && e.polygon([{ x: a[0].x, y: H2(t) }, ...a, { x: a[a.length - 1].x, y: H2(t) }], { fill: n.fill });
  const o = { stroke: n.gray ?? h.strong, width: n.width ?? 1.5 };
  if (n.smooth ? e.spline(a, o) : e.polyline(a, o), n.dot !== false) {
    const c = a[a.length - 1];
    e.circle(c.x, c.y, 2.2, { fill: h.off }), e.circle(c.x, c.y, 2.2, { stroke: n.gray ?? h.max, width: 1.2 });
  }
}
function le(e, t, i, n = {}) {
  const r = n.min ?? Math.min(0, ...i), s = n.max ?? Math.max(...i, 0), a = _2(e, t, r, s, { ...n, baseline: n.baseline ?? false }), o = n.gap ?? 2, c = (a.width - o * (i.length - 1)) / i.length, l = z(0, r, s, H2(a), a.y);
  i.forEach((f, u) => {
    const L = z(f, r, s, H2(a), a.y), x = a.x + u * (c + o), b2 = Math.abs(L - l), d = { x, y: Math.min(L, l), width: c, height: Math.max(1, b2) }, C = n.radius ?? Math.min(2, c / 3), V = u === n.highlight;
    v2(e) && !V ? e.roundRect(d, C, { stroke: n.gray ?? h.border, width: 1 }) : e.roundRect(d, C, { fill: V ? n.highlightGray ?? h.max : n.gray ?? h.secondary });
  }), r < 0 && e.hline(a.x, w2(a), l, h.border);
}
function fe(e, t, i, n = {}) {
  ...
  i.forEach((c, l) => {
    ...
    v2(e) ? e.roundRect(u, 3, { stroke: h.hairline, width: 1 }) : e.roundRect(u, 3, { fill: h.sunken }), e.roundRect({ ...u, width: Math.max(2, u.width * Z2(c.value, 0, r) / r) }, 3, { fill: c.gray ?? h.strong }), e.text(n.format ? n.format(c.value) : String(c.value), w2(t), f + o / 2, m.numeral, h.primary, "right", "middle");
  });
}
function de(e, t, i, n = {}) {
  const r = i.map((u) => u.y), s = n.min ?? Math.min(...r), a = n.max ?? Math.max(...r), o = _2(e, t, s, a, n), c = i.map((u) => u.x), l = n.xMin ?? Math.min(...c), f = n.xMax ?? Math.max(...c);
  for (const u of i)
    e.circle(
      z(u.x, l, f, o.x, w2(o)),
      z(u.y, s, a, H2(o), o.y),
...
function ue(e, t, i, n, r, s = {}) {
  const a = s.width ?? 6, o = s.startAngle ?? -Math.PI / 2, c = Z2(r, 0, 1);
  e.arc(t, i, n, 0, Math.PI * 2, { stroke: s.trackGray ?? (v2(e) ? h.hairline : h.sunken), width: v2(e) ? 1 : a, cap: "butt" }), c > 1e-3 && e.arc(t, i, n, o, o + Math.PI * 2 * c, { stroke: s.gray ?? h.max, width: a, cap: "round" });
  const l = n - a / 2 - 4;
  if (s.value) {
    const f = ht(e.measure, s.value, m.numeralLg, l * 1.9, l * 1.3);
    e.text(s.value, t, i - (s.label ? 5 : 0), f, h.max, "center", "middle");
  }
  s.label && e.text(Y(e.measure, s.label, m.micro, l * 1.9), t, i + 11, { ...m.micro, size: 8 }, h.tertiary, "center", "middle");
}
function me(e, t, i, n, r, s = {}) {
  const a = s.min ?? 0, o = s.max ?? 100, c = Math.PI * 0.75, l = Math.PI * 1.5, f = (x) => c + (Z2(x, a, o) - a) / (o - a) * l;
...
function ye(e, t, i, n, r = {}) {
  ...
  v2(e) ? e.roundRect(t, t.height / 2, { stroke: h.hairline, width: 1 }) : e.roundRect(t, t.height / 2, { fill: h.sunken }), e.roundRect({ ...t, width: Math.max(2, t.width * Z2(i, 0, s) / s) }, t.height / 2, { fill: r.gray ?? h.strong });
  const a = t.x + t.width * Z2(n, 0, s) / s;
  e.vline(a, t.y - 2, H2(t) + 2, h.max, 2);
}
function pt(e, t, i = {}) {
  const n = i.radius ?? A.lg, r = v2(e), s = i.fill ?? (r ? void 0 : h.surface), a = i.stroke ?? (r ? h.border : void 0);
  return s !== void 0 && e.roundRect(t, n, { fill: s }), i.texture && !r && e.scoped((o) => {
    o.clipRound(t, n), o.dots(t, 5, h.sunken, 0.5);
  }), a !== void 0 && e.roundRect(t, n, { stroke: a, width: i.strokeWidth ?? 1 }), B(t, O2.md);
}
function we(e, t, i, n = {}) {
  pt(e, t, { fill: n.fill, stroke: n.stroke, radius: n.radius ?? A.lg, texture: n.texture });
  const r = t.y + 13;
  let s = t.x + O2.md;
  n.icon && (R2(e, n.icon, s + 5, r, 12, h.tertiary), s += 17);
  const a = n.accessory ? e.measure(n.accessory, m.micro) + O2.sm : 0, o = w2(t) - O2.md - a - s;
  return e.text(Y(e.measure, i, m.label, o), s, r, m.label, h.secondary, "left", "middle"), n.accessory && e.text(n.accessory, w2(t) - O2.md, r, m.micro, h.tertiary, "right", "middle"), e.hline(t.x + O2.md, w2(t) - O2.md, t.y + 24, h.hairline), B({ x: t.x, y: t.y + 26, width: t.width, height: t.height - 26 }, { left: O2.md, right: O2.md, bottom: O2.md });
}
function ge(e, t, i, n = h.hairline) {
  const r = T2(t);
  if (!i) {
    e.hline(t.x, w2(t), r, n);
    return;
  }
  const s = e.measure(i, m.micro), a = 6;
  e.hline(t.x, I2(t) - s / 2 - a, r, n), e.hline(I2(t) + s / 2 + a, w2(t), r, n), e.text(i, I2(t), r, m.micro, h.tertiary, "center", "middle");
}
function Le(e, t, i, n = {}) {
  const r = n.align ?? "left", s = n.valueStyle ?? m.numeralLg, a = r === "left" ? t.x : r === "right" ? w2(t) : I2(t), o = H2(t) - 5, c = !!n.label, l = c ? H2(t) - 18 : T2(t);
  let f = a;
  n.icon && r === "left" && (R2(e, n.icon, t.x + 7, t.y + 8, 14, h.tertiary), f = t.x + 18);
  const u = n.unit ? e.measure(n.unit, m.caption) + 6 : 0, L = ht(
    e.measure,
    i,
    s,
    Math.max(24, w2(t) - f - u),
    c ? t.height - 24 : t.height
  ), x = e.measure(i, L);
  if (e.text(i, f, l, L, n.gray ?? h.max, r, "bottom"), n.unit) {
    const b2 = r === "right" ? f - x - 3 : r === "center" ? f + x / 2 + 3 : f + x + 3;
    e.text(n.unit, b2, l - 1, m.caption, h.tertiary, r === "right" ? "right" : "left", "bottom");
  }
  if (n.label && e.text(n.label, a, o, m.label, h.secondary, r, "bottom"), n.delta !== void 0 && n.delta !== 0) {
    const b2 = n.delta > 0;
    R2(e, b2 ? "caret-up" : "caret-down", w2(t) - 8, t.y + 9, 12, h.strong), n.deltaText && e.text(n.deltaText, w2(t) - 17, t.y + 9, m.micro, h.secondary, "right", "middle");
  }
}
function xe(e, t, i, n, r = {}) {
  const s = r.align ?? "left", a = s === "left" ? t.x : s === "right" ? w2(t) : I2(t);
  e.text(i, a, t.y, m.micro, h.tertiary, s, "top"), e.text(n, a, t.y + 12, r.style ?? m.numeral, r.gray ?? h.primary, s, "top");
}
function Ce(e, t, i, n, r = {}) {
  const s = T2(t);
  if (e.text(i, t.x, s, m.caption, h.secondary, "left", "middle"), e.text(n, w2(t), s, m.numeral, r.gray ?? h.primary, "right", "middle"), r.leader) {
    const a = e.measure(i, m.caption), o = e.measure(n, m.numeral), c = t.x + a + 5, l = w2(t) - o - 5;
    l > c && e.line(c, s + 2, l, s + 2, { stroke: h.hairline, width: 1, dash: [1, 3], cap: "butt" });
  }
}
function He(e, t, i) {
  i.selected && (v2(e) ? (e.roundRect({ x: t.x, y: t.y + 2, width: 2.5, height: t.height - 4 }, 1.5, { fill: h.max }), e.roundRect(t, A.md, { stroke: h.hairline, width: 1 })) : e.roundRect(t, A.md, { fill: h.raised }));
  const n = T2(t);
  let r = t.x + O2.md;
  i.checked !== void 0 && (R2(e, i.checked ? "check-square" : "square", r + 7, n, 15, i.checked ? h.max : h.disabled), r += 24), i.icon && (R2(e, i.icon, r + 8, n, 16, i.selected ? h.max : h.strong), r += 26);
  const s = w2(t) - O2.md - (i.chevron ? 14 : 0), a = i.value ? e.measure(i.value, m.numeral) + 10 : 0, o = s - r - a;
  i.subtitle ? (e.textBox(i.title, { x: r, y: n - 15, width: o, height: 16 }, m.bodyStrong, i.selected ? h.max : h.primary, { wrap: false, vAlign: "middle" }), e.textBox(i.subtitle, { x: r, y: n + 1, width: o, height: 14 }, m.caption, h.tertiary, { wrap: false, vAlign: "middle" })) : e.textBox(i.title, { x: r, y: t.y, width: o, height: t.height }, m.body, i.selected ? h.max : h.primary, { wrap: false, vAlign: "middle" }), i.value && e.text(i.value, s, n, m.numeral, i.selected ? h.max : h.secondary, "right", "middle"), i.chevron && R2(e, "chevron-right", w2(t) - O2.md, n, 12, h.disabled);
}
function Ve(e, t, i, n = {}) {
  const r = Z2(i, 0, 1), s = n.radius ?? t.height / 2;
  if (n.track !== void 0 ? e.roundRect(t, s, { fill: n.track }) : v2(e) ? e.roundRect(t, s, { stroke: h.hairline, width: 1 }) : e.roundRect(t, s, { fill: h.sunken }), n.ticks)
    for (let a = 1; a < n.ticks; a++) e.vline(t.x + t.width * a / n.ticks, t.y, H2(t), h.off);
...
function pe(e, t, i, n, r = {}) {
  ...
  for (let o = 0; o < i; o++) {
    ...
    o < n ? e.roundRect(c, l, { fill: r.fill ?? h.max }) : v2(e) && r.track === void 0 ? e.roundRect(c, l, { stroke: h.hairline, width: 1 }) : e.roundRect(c, l, { fill: r.track ?? h.sunken });
  }
}
function kt(e, t, i, n, r = {}) {
  const s = r.width ?? 22, a = r.height ?? 11, o = Z2(n, 0, 1);
...
function ke(e, t, i = {}) {
  ...
  for (const s of i.icons ?? [])
    R2(e, s, r - 7, n, 13, h.secondary), r -= 18;
...
function be(e, t, i, n) {
  ...
  i.forEach((s, a) => {
    ...
    e.text(s, o, T2(t) - 2, m.micro, c ? h.max : h.disabled, "center", "middle"), c && e.roundRect({ x: o - 12, y: H2(t) - 4, width: 24, height: 2 }, 1, { fill: h.max });
  });
}
function Oe(e, t, i, n, r) {
  if (r <= n) return;
  v2(e) || e.roundRect(t, t.width / 2, { fill: h.sunken });
  const s = Math.max(10, n / r * t.height), a = t.y + i / r * t.height;
  e.roundRect({ x: t.x, y: Math.min(a, H2(t) - s), width: t.width, height: s }, t.width / 2, { fill: h.border });
}
function Ze(e, t, i, n, r = {}) {
  ...
  return r.fill !== void 0 ? e.roundRect(l, a / 2, { fill: r.fill }) : v2(e) ? e.roundRect(l, a / 2, { stroke: h.border, width: 1 }) : e.roundRect(l, a / 2, { fill: h.raised }), r.icon && R2(e, r.icon, t + 12, i, 11, r.gray ?? h.secondary), e.text(n, t + 8 + o, i, s, r.gray ?? h.primary, "left", "middle"), c;
}
...
function Ee(e, t, i, n = {}) {
  const r = v2(e), s = r ? void 0 : n.primary ? h.max : h.raised, a = r ? n.primary ? h.max : h.secondary : n.primary ? h.off : h.primary;
  ...
  const o = n.icon ? 18 : 0, c = e.measure(i, m.bodyStrong), l = I2(t) - (c + o) / 2;
  n.icon && R2(e, n.icon, l + 7, T2(t), 14, a), e.text(i, l + o, T2(t), m.bodyStrong, a, "left", "middle");
}
function Ae(e, t, i, n, r = 32) {
  const s = r * 0.56;
  v2(e) ? (e.roundRect({ x: t, y: i - s / 2, width: r, height: s }, s / 2, { stroke: n ? h.max : h.border, width: 1 }), e.circle(n ? t + r - s / 2 : t + s / 2, i, s / 2 - 3, { fill: n ? h.max : h.disabled })) : (e.roundRect({ x: t, y: i - s / 2, width: r, height: s }, s / 2, { fill: n ? h.max : h.border }), e.circle(n ? t + r - s / 2 : t + s / 2, i, s / 2 - 2.5, { fill: n ? h.off : h.surface }));
}
function Se(e, t, i, n = {}) {
  e.scoped((r) => {
    v2(e) || r.roundRect(t, A.pill, { fill: h.raised, alpha: n.alpha ?? 1 }), r.roundRect(t, A.pill, { stroke: h.border, width: 1, alpha: n.alpha ?? 1 });
    let s = t.x + 16;
    n.icon && (R2(r, n.icon, s + 7, T2(t), 14, h.max), s += 24), r.text(i, s, T2(t), m.body, h.primary, "left", "middle");
  });
}
function Ie(e, t, i, n, r) {
  const s = T2(t) - (r ? 10 : 4);
  R2(e, i, I2(t), s - 18, 30, h.disabled), e.text(n, I2(t), s + 10, m.title, h.secondary, "center", "middle"), r && e.textBox(r, { x: t.x + 40, y: s + 22, width: t.width - 80, height: 32 }, m.caption, h.tertiary, { hAlign: "center" });
}
function Ge(e, t, i = 0.5) {
  e.rect(t, { fill: h.off, alpha: i });
}
function Pe(e, t, i, n, r, s = {}) {
  if (e.circle(t, i, n, { stroke: h.border, width: 1 }), s.ticks !== false)
    for (let o = 0; o < 360; o += 15) {
      const c = q2(o - r), l = o % 45 === 0, f = E2(t, i, n - (l ? 7 : 4), c), u = E2(t, i, n - 1, c);
      e.line(f.x, f.y, u.x, u.y, { stroke: l ? h.secondary : h.hairline, width: l ? 1.5 : 1, cap: "butt" });
    }
  const a = [["N", 0], ["E", 90], ["S", 180], ["W", 270]];
  for (const [o, c] of a) {
    const l = E2(t, i, n - 16, q2(c - r));
    e.text(o, l.x, l.y, o === "N" ? m.bodyStrong : m.caption, o === "N" ? h.max : h.secondary, "center", "middle");
  }
  if (s.pointer !== void 0) {
    const o = q2(s.pointer - r), c = E2(t, i, n - 22, o), l = E2(t, i, n - 33, o - 0.16), f = E2(t, i, n - 33, o + 0.16);
    if (e.polygon([c, l, f], { fill: h.strong }), s.pointerLabel) {
      const u = E2(t, i, n - 40, o);
  ...
  e.polygon(
    ...
    { fill: h.max }
  ), s.showValue !== false && e.text(`${Math.round(r).toString().padStart(3, "0")}°`, t, i, m.numeralLg, h.max, "center", "middle");
}
function ze(e, t, i, n, r, s) {
  e.scoped((a) => {
    ...
    v2(e) || a.rect(c, { fill: h.sunken }), a.hatch(c, 5, v2(e) ? h.hairline : h.border), a.hline(t - n * 1.4, t + n * 1.4, o, h.max, 1.5);
...
function Fe(e, t, i, n, r, s, a = "kn") {
  ...
  const o = q2(r + 180), c = E2(t, i, n, o), l = E2(t, i, -n * 0.75, o);
  e.line(l.x, l.y, c.x, c.y, { stroke: h.strong, width: 2 });
  const f = E2(c.x, c.y, n * 0.35, o + 2.5), u = E2(c.x, c.y, n * 0.35, o - 2.5);
...
function $e(e, t, i, n, r, s = false) {
  s && e.circle(t, i, n + 4, { stroke: h.strong, width: 1.5, dash: [3, 4] }), v2(e) ? e.circle(t, i, n - 1.75, { stroke: h.max, width: 3.5 }) : (e.circle(t, i, n, { fill: h.max }), e.circle(t, i, n - 3.5, { fill: h.off })), e.text(String(r), t, i + 1, { ...m.numeralLg, size: n * 1.05 }, h.max, "center", "middle");
}
function Be(e, t, i, n, r) {
  R2(e, r === "left" ? "corner-left" : r === "right" ? "corner-right" : r === "uturn" ? "u-turn" : r === "slight-left" ? "arrow-up-left" : r === "slight-right" ? "arrow-up-right" : "arrow-up", t, i, n, h.max, { width: Math.max(2, n / 12) });
}
var We = class {
  width;
  height;
  ...
  suggest(t = {}) {
    ...
    const i = t.step ?? 16, n = t.minTile ?? 32, r = Math.min(t.tiles ?? $, $);
    ...
    const s = /* @__PURE__ */ new Map(), a = (d) => {
      ...
      if (!(V >= o.cost)) {
        ...
        for (let y = i; y <= d.w - i; y += i) {
          ...
          const M2 = { x: d.x, y: d.y, w: y, h: d.h }, p = { x: d.x + y, y: d.y, w: d.w - y, h: d.h };
          for (let g = 1; g < C; g++) f(M2, g, p, C - g, V);
        }
        for (let y = i; y <= d.h - i; y += i) {
          if (y < n || d.h - y < n) continue;
          const M2 = { x: d.x, y: d.y, w: d.w, h: y }, p = { x: d.x, y: d.y + y, w: d.w, h: d.h - y };
          for (let g = 1; g < C; g++) f(M2, g, p, C - g, V);
        }
      }
    }, f = (d, C, V, y, M2) => {
      ...
      for (let M2 = i; M2 <= d.w - i; M2 += i) {
        if (M2 < n || d.w - M2 < n) continue;
        const p = { x: d.x, y: d.y, w: M2, h: d.h }, g = { x: d.x + M2, y: d.y, w: d.w - M2, h: d.h };
        for (let k = 1; k < C; k++)
          u(p, k, V, (P, G) => u(g, C - k, G, (F2, W2) => y([...P, ...F2], W2)));
      }
      for (let M2 = i; M2 <= d.h - i; M2 += i) {
        if (M2 < n || d.h - M2 < n) continue;
        const p = { x: d.x, y: d.y, w: d.w, h: M2 }, g = { x: d.x, y: d.y + M2, w: d.w, h: d.h - M2 };
        for (let k = 1; k < C; k++)
          u(p, k, V, (P, G) => u(g, C - k, G, (F2, W2) => y([...P, ...F2], W2)));
      }
    };
    ...
    const x = bt(L, t.name ?? "measured"), b2 = Z(x, this.width, this.height);
    if (b2.length) throw new Error(`Glyph: search produced an invalid layout:
  ${b2.join(`
  `)}`);
    return this.cost(x);
  }
...
var Ne = class {
  ...
  constructor(t) {
    ...
    this.screens = t.screens, this.frame = new rt({
      width: $2.width,
      height: $2.height,
      supersample: t.supersample ?? 2,
      tileLayout: t.tileLayout ?? I,
  ...
  set surface(t) {
    this.frame.raster.surface = t, this.fromLayer && (this.fromLayer.surface = t), this.toLayer && (this.toLayer.surface = t), this.runtime?.invalidate(), this.diff.reset(), this.dirty = true;
  }
  ...
  set brightness(t) {
    this.frame.raster.brightness = t, this.runtime?.invalidate(), this.diff.reset(), this.dirty = true;
  }
  /** Install a bitmap font set on this app's surface and its transition layers. */
  useFont(t, i = t ? "auto" : "canvas") {
    this.frame.raster.useFont(t, i), this.fromLayer?.useFont(t, i), this.toLayer?.useFont(t, i), this.runtime?.invalidate(), this.diff.reset(), this.dirty = true;
  }
  ...
  attachRuntime(t) {
    this.runtime = t, t?.invalidate(), this.dirty = true;
  }
  ...
  goto(t, i) {
    ...
    if (this.transitionMs > 0) {
      ...
      s.clear(0), s.drawRaster(this.frame.raster), this.ensureLayer("to"), this.transition = {
        tween: new it2(0, this.transitionMs, et2.inOutCubic).to(1, this.now),
        direction: r
      };
    }
    this.screen.onExit?.(this), this.index = n, this.screen.onEnter?.(this), this.dirty = true;
  }
  ensureLayer(t) {
  ...
  handleInput(t) {
    if (this.screen.onInput?.(t, this) === true) {
      this.dirty = true;
      return;
    }
    t.type === "tap" || t.type === "scroll-down" ? this.next() : t.type === "scroll-up" && this.previous(), this.dirty = true;
  }
  ...
  paint() {
    ...
    if (this.transition && this.fromLayer && this.toLayer) {
      const o = Z2(this.transition.tween.valueAt(i), 0, 1), { direction: c } = this.transition;
      this.toLayer.clear(0), this.screen.render(n(this.toLayer));
      const l = this.frame.raster;
      l.clear(0), l.drawRaster(this.fromLayer, -c * $2.width * o, 0), l.drawRaster(this.toLayer, c * $2.width * (1 - o), 0), o >= 1 && (this.transition = null);
    } else
  ...
  tick = () => {
    ...
    const i = this.screen.animated === true || this.transition !== null;
    if (!this.dirty && !i) return;
    const n = t - this.lastPaint;
    this.lastPaint = t, this.fps = this.fps === 0 ? 1e3 / n : this.fps * 0.9 + 1e3 / n * 0.1, this.dirty = false, this.paint();
  };
  start() {
    this.running || (this.running = true, this.started = performance.now(), this.lastPaint = performance.now() - this.interval, this.screen.onEnter?.(this), this.frameHandle = requestAnimationFrame(this.tick));
  }
  stop() {
    this.running = false, this.frameHandle !== null && cancelAnimationFrame(this.frameHandle), this.frameHandle = null;
  }
...
var je = class {
  ...
  offset = 0;
  move(t) {
    this.index = Z2(this.index + t, 0, Math.max(0, this.total - 1)), this.index < this.offset && (this.offset = this.index), this.index >= this.offset + this.visible && (this.offset = this.index - this.visible + 1);
  }
  setTotal(t) {
    this.total = t, this.index = Z2(this.index, 0, Math.max(0, t - 1)), this.offset = Z2(this.offset, 0, Math.max(0, t - this.visible));
  }
...
//# sourceMappingURL=@gabrielevierti_glyph.js.map
