...
- `--sdk-ver <version>` - Use the minimum app version required by the specified SDK release
- `--enforce-manual-version` - Stamp `min_app_version` from `app.json` as written, even when it is below the SDK requirement
...
evenhub pack app.json ./dist --sdk-ver 0.0.13
```

During packing, the CLI obtains the SDK's `minAppVersion` from npm and stamps
it into the packaged manifest. Without `--sdk-ver`, it uses the latest
published SDK. If `app.json` declares a higher `min_app_version`, that stricter
value is preserved; a lower value is raised to the SDK floor. The source
`app.json` remains unchanged.

If npm cannot be reached, the CLI warns and falls back to its bundled
release-time mapping. `--enforce-manual-version` requires `min_app_version` in
`app.json`. A value below the SDK requirement may cause the package to fail on
older app versions.

An `.ehpk` cannot currently be opened or run directly. To test one on a
device, upload it through the EvenHub site, then open it from the Even app on
your phone.

## Shell Completion

Scripts cover both `evenhub` and its `eh` alias.

```bash
# bash — source from .bashrc
evenhub --completion-bash > ~/.evenhub-completion.bash

# zsh — any directory on your fpath
evenhub --completion-zsh > ~/.zfunc/_evenhub

# fish — needs a second file, since fish autoloads by command name
evenhub --completion-fish > ~/.config/fish/completions/evenhub.fish
echo 'complete -c eh -w evenhub' > ~/.config/fish/completions/eh.fish
```
...
- derive and stamp `min_app_version` from SDK npm metadata during packing
- add `--sdk-ver` and the `--enforce-manual-version` compatibility override
- add shell completion support for both `evenhub` and `eh`
- return a failing exit status when packing fails
