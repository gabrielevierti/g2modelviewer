_evenhub() {
    ...
    case "${cmd}" in
        ...
        evenhub__login)
            ...
            ;;
        evenhub__pack)
            opts="-o -c -h --output --no-ignore --check --sdk-ver --enforce-manual-version --help <APP_JSON> <PROJECT_DIR>"
            if [[ ${cur} == -* || ${COMP_CWORD} -eq 2 ]] ; then
                COMPREPLY=( $(compgen -W "${opts}" -- "${cur}") )
                return 0
            fi
...
if [[ "${BASH_VERSINFO[0]}" -eq 4 && "${BASH_VERSINFO[1]}" -ge 4 || "${BASH_VERSINFO[0]}" -gt 4 ]]; then
    complete -F _evenhub -o nosort -o bashdefault -o default evenhub eh
else
    complete -F _evenhub -o bashdefault -o default evenhub eh
fi
