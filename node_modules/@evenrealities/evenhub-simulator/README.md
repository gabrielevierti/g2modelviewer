...
- **Supported Inputs:** Up, Down, Click, Double Click, Long Press, Long Press Release, Context Menu.
...
| `POST`   | `/api/input`               | Send a glasses touchpad action. JSON body: `{ "action": "up" \| "down" \| "click" \| "double_click" \| "long_press" \| "long_press_release" \| "context_menu" }`. |
...
- Prevent simulator crashes during high-frequency SDK bridge and console traffic.

## v0.9.4 (2026-08-31)

- Add Linux and Windows ARM64 release builds (experimental).

## v0.9.3 (2026-08-22)

- Match encoded-image grayscale conversion with the glasses.
- Match cover resizing and center cropping when image and container dimensions differ.

## v0.9.2 (2026-08-21)

- Support SDK raw Gray8 and packed Gray4 image data while preserving encoded-image decoding.

## v0.9.1 (2026-08-20)

- Add long-press and long-press-release actions to the automation API.

## v0.9.0 (2026-08-19)

- Add `textColor` rendering for text containers.
- Add long-press and long-press-release input simulation, including holdable
  controls and keyboard shortcuts.
- Add contextual menu simulation

## v0.8.0 (2026-07-03)

- Support `zOrderIndex` on containers: draw order is sorted across list/text/image
  containers by `zOrderIndex`. Matching the SDK, it must be set on all containers
  or none, and values must be unique within the page — partially-specified or
  duplicate payloads are rejected.
- Add an image decoding safety limit: decoded image pixel dimensions are now
  capped to prevent excessive memory use.
